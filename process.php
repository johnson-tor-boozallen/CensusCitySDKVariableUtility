...
$aliasList = json_decode($sfaliases,true);
$newAliasList = Array();

$str = file_get_contents('http://api.census.gov/data.json');
$allAPI = json_decode($str, true);

$allAPIVariables = Array();
$validApiYearList = Array();
foreach($allAPI['dataset'] as $apiDef){
    if(isset($apiDef['c_vintage'])){
    echo "\nLoading: ".$apiDef['c_variablesLink'];
        $startPos = strpos($apiDef['c_variablesLink'],(string)$apiDef['c_vintage']) + 5;
        $intermediate = substr($apiDef['c_variablesLink'],$startPos);
        $endPos = strrpos($intermediate,'/');
        $key = substr($intermediate,0,$endPos);

    $str = file_get_contents("cache/".$key.(string)$apiDef['c_vintage'].".json");
    if(!$str){
        $str = file_get_contents($apiDef['c_variablesLink']);
        file_put_contents("cache/".$key.(string)$apiDef['c_vintage'].".json",$str);

    }

    echo "\nKey: $key\n";

    $allAPIVariables[$key][(string)$apiDef['c_vintage']] = json_decode($str, true);

        // Create API-Year KeyList
        $validApiYearList[$key][]=(string)$apiDef['c_vintage'];

        // Create Required Variable List
        foreach($allAPIVariables[$key][(string)$apiDef['c_vintage']]['variables'] as $variableName => $variable){
            if(isset($variable['required'])){
                if($variable['required']==true){
                    $apiRequiredVariables[$key][(string)$apiDef['c_vintage']][]=$variableName;
                }
            }
        }
            // Hardcoded 'Required' Variables
            if(isset($allAPIVariables[$key][(string)$apiDef['c_vintage']]['variables']["NAME"]) && !in_array("NAME",$apiRequiredVariables[$key])){
                $apiRequiredVariables[$key][(string)$apiDef['c_vintage']][]="NAME";
            }elseif(isset($allAPIVariables[$key][(string)$apiDef['c_vintage']]['variables']["ANPSADPI"]) && !in_array("ANPSADPI",$apiRequiredVariables[$key])){
                $apiRequiredVariables[$key][(string)$apiDef['c_vintage']][] = "ANPSADPI";
            }
...
foreach($aliasList as $key => $alias){
    $alias['api']=Array();
    foreach($allAPIVariables as $apiKey => $apiSet){
        foreach($apiSet as $year =>$singleApi){
            if(isset($singleApi['variables'][$alias['variable']])){
                $alias['api'][$apiKey][]=$year;
            }
        }
    }
    $newAliasList[$key]=$alias;
}



echo "\n\n\n\n Writing Files...";
file_put_contents("aliases.json", json_encode($newAliasList));
file_put_contents("validYearSet.json", json_encode($validApiYearList));
file_put_contents("apiRequiredVariables.json", json_encode($apiRequiredVariables));
echo " Done\n\n\n";
